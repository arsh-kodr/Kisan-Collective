...
const Listings = () => {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setPending((p) => p.filter((l) => l._id !== id));
      const aRes = await listingApi.getListings();
      setApproved(aRes.data);
  ...
  return (
    <div className="bg-white rounded-xl shadow p-6">
      <div className="flex gap-4 mb-4 border-b">
        <button
          className={`pb-2 ${activeTab === "pending" ? "border-b-2 border-green-600 text-green-600" : "text-gray-500"}`}
          onClick={() => setActiveTab("pending")}
        >
          Pending
        </button>
        <button
          className={`pb-2 ${activeTab === "approved" ? "border-b-2 border-green-600 text-green-600" : "text-gray-500"}`}
      ...
      {data.length === 0 ? (
        ...
        <ul className="divide-y divide-gray-200">
          {data.map((listing) => (
            <li key={listing._id} className="flex justify-between items-center py-3 flex-wrap gap-2">
              <div>
                <p className="font-medium text-gray-800">{listing.crop}</p>
                <p className="text-xs text-gray-500">{listing.quantityKg} kg · {listing.location}</p>
              </div>
              {activeTab === "pending" && (
